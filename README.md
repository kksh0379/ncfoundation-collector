...
- **중복 제거: 본문 내용 유사도 (단어 TF-IDF + 코사인 유사도)** — 여러 매체가 같은
  보도자료를 배포하는 경우 1건만 저장. Kiwi 형태소 분석으로 본문을 의미 형태소로
  토큰화(동사/형용사는 원형 환원)한 뒤 TF-IDF 벡터화하여 코사인 유사도를 비교한다.
  (임계값 `SIMILARITY_THRESHOLD = 0.75`, `collector/dedup.py`)
