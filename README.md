...
### 탭1 · 뉴스 · 기사
- 구글 뉴스 RSS에서 **재단**(`엔씨문화재단`/`NC문화재단`)·**본사**(`엔씨소프트`/`NCSOFT`/`NC`/`엔씨` + 자회사 `NC AI`/`엔씨QA`/`엔씨IDS`/`퍼스트스파크·빅파이어·루디우스 게임즈`, NC다이노스 야구 기사는 제외)
  카테고리별로 수집. 뉴스 탭 상단 셀렉트로 전체/재단/본사 필터
- 최근 2년 기사
...
- **동일 기사 그룹화(아코디언)**: 같은 보도자료가 여러 매체에 배포된 경우를
  전부 수집하되, 본문/제목 유사도(글자 n-gram TF-IDF 코사인, 임계값 0.55)로
  묶어 `group_key`를 부여하고 화면에서 아코디언으로 접어 보여준다.
  (`collector/dedup.py`의 `cluster_items`)
