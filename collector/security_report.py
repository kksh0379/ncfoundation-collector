...
SYSTEM_PROMPT = (
    ...
    '"regulatory":[{"title":"처분/법제 제목","note":"요점 1문장","org":"개보위|공정위|기타(기관)","penalty":"과징금/과태료 금액(있으면)","target":"대상 기업/기관(있으면)"}],'
    '"trends":[{"topic":"보안 트렌드","note":"1문장"}],'
    '"actions":[{"task":"이번 달 점검/대응 권고","detail":"구체 실행 1문장"}],'
    '"confidence_note":"데이터 한계 안내 1문장"}\n'
    "regulatory에는 특히 **개인정보보호위원회(개보위)·공정거래위원회(공정위)의 처분·의결**"
    "(과징금·과태료 금액, 위반 내용, 대상 기관)을 최우선으로 포함한다. 금액·대상이 기사에 없으면 해당 필드는 빈값.\n"
    "confidence_note는 데이터 한계를 정확히 쓴다: 개보위·공정위 관련 발표를 '언론 보도 기준'으로 반영했으며, "
    "공식 의결서 전문·기술적 침해 원인 상세는 원문(개보위/기관 발표) 확인이 필요함을 명시한다.\n"
    "top_incidents는 최대 6개, regulatory는 최대 6개, 나머지 배열은 각각 최대 5개. 근거 없는 내용은 넣지 않는다."
)
