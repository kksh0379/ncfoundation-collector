...
# 한글 "엔씨" + 영문 "NC AI/NCSOFT/NC문화재단" 등 라틴 표기까지 모두 걸러낸다.
BIZ_EXCLUDE_BODY = [
    "엔씨",  # 엔씨소프트/엔씨문화재단/엔씨에이아이/엔씨qa/엔씨ids 등 한글 표기 전부
    "ncsoft", "nc소프트", "nc ai", "ncai", "nc qa", "nc ids",
    "nc문화재단", "ncfoundation", "nc다이노스", "nc dinos",
    "퍼스트스파크", "빅파이어", "루디우스",  # NC 게임 자회사
]
...
SECURITY_CATEGORIES = {
    ...
    "정책·규제": [
        "(개인정보보호법 OR ISMS OR \"ISMS-P\" OR \"정보보호 공시\" OR KISA) "
        "(개정 OR 시행 OR 가이드라인 OR 고시 OR 발표 OR 의무화)",
    ],
    # 개보위·공정위 등 규제기관의 '공식 처분·의결' 중심(담당자 핵심). 보도 기준으로 최대한 포착.
    "개보위·처분": [
        "(개인정보보호위원회 OR 개인정보위) (의결 OR 처분 OR 과징금 OR 과태료 OR 시정명령 "
        "OR 시정권고 OR 조사 OR 제재 OR 전체회의 OR 심의 OR 유출조사)",
        "(공정거래위원회 OR 공정위) (개인정보 OR 다크패턴 OR 소비자보호 OR 과징금 OR 심의)",
    ],
    "보안트렌드": [
